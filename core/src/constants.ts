export const RECEIVE_MESSAGE_FROM_FEED = 'cevitxe/RECEIVE_MESSAGE_FROM_FEED'

export const INITIALIZE = 'cevitxe/INITIALIZE'

export const MSG_INVALID_KEYS =
  'A valid crypto pair, `key` and a `secretKey`, must be provided. ' +
  'The `key` should be a 32-byte hexadecimal value (64 characters). ' +
  'The `secretKey` should be a 64-byte hexadecimal value (128 characters). ' +
  'These can be generated by running `yarn keys` from the cevitxe module.'

export const DEFAULT_PEER_HUBS = ['https://signalhub-jccqtwhdwc.now.sh/'] // default public signaling server

export const DOC_ID = '1'
